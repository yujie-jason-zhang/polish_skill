...
- Check whether the same author appears with inconsistent name order, spelling, initials, capitalization, or transliteration. Pay particular attention to possible Chinese-name order inconsistencies, where the family name may appear first in one entry and last in another, such as `Zhang SanSi`, `SanSi Zhang`, `Zhang, S.`, and `S. Zhang` potentially referring to the same author. Report the suspected inconsistency for user review instead of normalizing it automatically.
